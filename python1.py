...
● Ako se nalazi na početku izvodi se u vremenu O(n) – treba pomjeriti sve
● elemente udesno
...
● U gornjem primjeru prvo ce se izvrsiti nivo 3 podizraz a to su: (8-6) i (45+60). Ako su istog nivoa
...
● STAVI-NA-STEK (S, x) - U prvom koraku se alocira prostor za novi čvor (linija 1), a zatim se u
informacioni dio upisuje novi element x (linija 2). Na kraju se novi čvor povezuje sa prethodnim
čelom liste, dok se pokazivač vrh ažurira tako da pokazuje na novi čvor (linije 3-4
...
• SKINI-SA-STEKA (S) - Ako pokazivač vrh ima vrijednost NIL, signalizira se da je stek prazan (linije 1- 3).
U suprotnom, u privremeni objekat x se upisuje informacioni sadržaj elementa na čelu liste (linija 4), te
se u privremenom pokazivaču p čuva adresa trenutnog čela liste (linija 5). Nakon toga se ažurira
pokazivač vrh, tako da pokazuje na sljedbenika čvora na čelu (linija 6), čime se u logičkom smislu
element sa vrha steka izbacuje iz liste. Na kraju je potrebno taj čvor i fizički ukloniti (linija 7), te vratiti
informacioni sadržaj skinutog elementa (linija 8).
SKINI-SA-STEKA(S)
if(vrh[S] = NIL) then
...
sa g(n)
• Može se sa sigurnošću tvrditi da f(n) nema veći red veličine rasta nego g(n). Napomenimo da može
...
• Za funkciju f(n) kažemo da je Ω(g(n)), ako postoje pozitivne konstante c i n0 tako da je ispunjeno:
f n ≥ cg n , ∀ n ≥ n0
NOTACIJA BIG-Θ
...
• Za funkciju f(n) kažemo da je Θ(g(n)), ako postoje pozitivne konstante c1, c2 i n0 tako da je
ispunjeno:
c1g n ≤ f n ≤ c2g n , ∀ n ≥ n0
• Može se zaključiti da je f(n) = Θ(g(n)) ako je ujedno, f(n) = O(g(n)) i f(n)= Ω(g(n))


5. Opisati procedure umetanja sa steka uz obrazlozenje funkcije STAVI-NA-STEK, uz pomoc povezanih listi.
● STAVI-NA-STEK (S, x) - U prvom koraku se alocira prostor za novi čvor (linija 1), a zatim se u informacioni dio upisuje novi element x (linija 2). Na kraju se novi čvor povezuje sa prethodnim čelom liste, dok se pokazivač vrh ažurira tako da pokazuje na novi čvor (linije 3-4
STAVI-NA-STEK(S,x)
p🡨GETNODE()
info(p) 🡨 x
sljedeci(p) 🡨vrh[S]
...
if (pocetak[Q] = (kraj[Q] + 1) mod duzina[Q]) then
	...
	return false
end_if
Operacija umetanja novog elementa x u red Q je opisana funkcijom STAVI-U-RED. • Prvo se pozivom funkcije JE-LI-PUN(Q) provjerava da li je red Q pun (linija 1). Ako je red pun, novi element se ne može umetnuti, te se signalizira prekoračenje kapaciteta reda (linija 2). Nadalje, ako red nije pun, povećava se pokazivač kraj za 1, tako da pokazuje na prvu sljedeću slobodnu lokaciju u nizu, te se na tu lokaciju upisuje novi element x (linije 4 i 5). Na kraju, procedura STAVI-U-RED još provjerava da li se umetanje obavlja u prazan red, jer se u toj situaciji treba, pored oznake kraj, ažurirati i oznaka pocetak. Naime, ako je red bio prazan (pocetak=-1), onda se postavlja i oznaka čela reda pocetak navrijednost 0 (linije 6-8)
...
• Alokacija po redovima - Provodi se na način da se prvo smješta prvi red, zatim drugi red sve do reda m-1. Ako se koristi raspon i=l1,...u1 i j=l2,...u2, adresna funkcija za element X[i, j] ima oblik Ai,j = Al1,l2 +(( i − l1) ∙ n + j − l2) ∙ s l1 ≤ i ≤ u1 i l2 ≤ j ≤ u2 gdje je Al1,l2 adresa prvog elementa, m broj redova u1-l1+1, n broj kolona u2-l2+1 i s memorijska veličina jednog elementa 
• Alkacija po kolonama - Provodi se na način da se prvo smješta prva kolona, zatim druga kolona sve do reda n-1. Ako se koristi raspon i=l1,...u1 i j=l2,...u2, adresna funkcija za element X[i, j] ima oblik Ai,j = A0,0 + (j ∙ m + i) ∙ s i = 0, 1, ... , m − 1 i j = 0, 1, ... , n − 1 gdje je Al1,l2 adresa prvog elementa, m broj redova u1-l1+1, n broj kolona u2-l2+1 i s memorijska veličina jednog elementao
